 ...
 *
/*--------------------------------------------------------------------------*/
var IMAGE_SRC_URL = "../images";
//var IMAGE_SRC_URL = "http://116.90.86.28:802/pm02/images";
var Prototype = {
  Version: '1.5.0',
  BrowserFeatures: {
		...
		onSuccess: function(t) {
			...
			if(n == 10)
			{
				...
				else
				{
					...
					//alert('../function/getTask.php?'+str+'&type=getDo');
					//window.status = '../function/getTask.php?'+str+'&type=getDo';return false;
...
function complatetask(str)
{
	...
	var ajax=new Ajax.Request('../function/getTask.php?'+str+'&type=complate', opt);	
}
// open map
function openMap(n)
{
...
if (!window.createPopup) { 
	var __createPopup = function() { 
		...
		{ 
			...
			for ( var styleName in styleDict )
				element.style[ styleName ] = styleDict[ styleName ] ; 
		}; 
		var eDiv = document.createElement( 'div' ) ; 
		SetElementStyles( 
						 eDiv, { 'position': 'absolute',
						 'top': 0 + 'px', 'left': 0 + 'px',
						 'width': 0 + 'px', 'height': 0 + 'px',
						 'zIndex': 200, 'display' : 'none', 'overflow' : 'hidden' } 
						 ) ; 
		eDiv.body = eDiv ; 
		var opened = false ; 
	...
	window.createPopup = function() { return __createPopup(); } ; 
} 
...
function gameTips(msg)
{
	...
	with (oPopup.document.body) 
	{
		...
		//style.background=src:'../images/images/ui/jg/jg04.jpg';
		//style.background-image:='../images/images/ui/jg/jg04.jpg';
	...
	{
		oPopup.show(70, 270, 400, 300);
	}
}

function taskCatch(n)
{
	var opt = {
     	method: 'get',
		onSuccess: function(t) {
			 		if(t.responseText!='') gameTips(t.responseText);
    		 	},
     	asynchronous:true        
	}
	var ajax=new Ajax.Request('../function/getTaskinfo.php?n='+n, opt);	
}

function taskCatchs(tid, n, t)
{
	var opt = {
     	method: 'get',
		onSuccess: function(t) {
	...
	var ajax=new Ajax.Request('../function/getTaskinfo.php?op=1&n='+n+'&t='+tid, opt);
}
//完成任务
function taskCatchss(tid, n, t)
{
	var opt = {
     	method: 'get',
		onSuccess: function(t) {
	...
	var ajax=new Ajax.Request('../function/getTaskinfo.php?op=2&n='+n+'&t='+tid, opt);
}


	//屏蔽右键
	document.oncontextmenu=myContextMenu;
	function myContextMenu(para,props_id)
	{
		var mid_id = para;
		if(typeof(para) == 'object')
		{
			if(para.target.className == 'p2')
			{
				para = para.target.innerHTML;
				if(typeof(props_id) == 'undefined')
				{
					props_id = mid_id.target.id.substring(1);
				}
			}
			else
			{
		...
		{
			if(event.srcElement.className == 'p2')
			{
				para = event.srcElement.innerHTML;
				if(typeof(props_id) == 'undefined')
				{
					props_id = event.srcElement.id.substring(1);
				}
			}
			else
			{
		...
		try{
				//window.parent.$('cmsg').value+=' {'+robj.innerHTML+'} ';
