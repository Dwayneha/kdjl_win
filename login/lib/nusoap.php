<?php
@session_start();
$lys_id = isset($_SESSION['lys_id'])?$_SESSION['lys_id']:"webgame";
$_SESSION['lys_id'] = $lys_id;
$sql = " SELECT * FROM T_udcconfig WHERE F_lys_id = '".$lys_id."'";
$udcresult = $_pm['mysql'] -> getOneRecord($sql);
if( empty($udcresult) )
{
	die("no udc infomation");
}
global $udcAcccount;
$udcAcccount = $udcresult['F_udcacccount'];
global $udcPassword;
$udcPassword = $udcresult['F_udcpassword'];
if( !empty($udcresult['F_prefix']) && !$_SESSION['prefix'])
{
	$_SESSION['prefix'] = $udcresult['F_prefix'];
	$_SESSION['username'] = $_SESSION['prefix'].$_SESSION['username'];
}
global $url;
include_once('udc_url.php');
function queryCoin($account,$licenseId,$lys_id='webgame')
{
	global $udcAcccount;
	global $udcPassword;
	global $url;
	$requestService='userQueryCoin';
	$client = new soapclientw($url, false,'','', '','');
	$err = $client->getError();
	$account=str_replace("]]>","]]&gt;",$account);
	$licenseId=str_replace("]]>","]]&gt;",$licenseId);
	if( $lys_id == 'webgame' )
	{
		$params ='<?xml version="1.0" encoding="GB2312"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService><game_code><![CDATA[kdjl]]></game_code></SOAP-ENV:Header><SOAP-ENV:Body><user_account><![CDATA['.$account.']]></user_account><license_id><![CDATA['.$licenseId.']]></license_id></SOAP-ENV:Body></SOAP-ENV:Envelope>';
	}
	else
	{
		$account = str_replace($_SESSION['prefix'],'',$account);
		$params ='<?xml version="1.0" encoding="GB2312"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><game_code><![CDATA[kdjl]]></game_code><user_account><![CDATA['.$account.']]></user_account><ly_pid><![CDATA['.$lys_id.']]></ly_pid></SOAP-ENV:Body></SOAP-ENV:Envelope>';
	}
	$result = $client->call('ManufacturerSearchRequest', $params, $url, 'http://soap.amazon.com');
	return $result;
}

// Return: 10->success. other:false.
function payment($account,$licenseId,$order_id,$feeMoney,$subject,$lys_id='webgame')
{
	global $udcAcccount;
	global $udcPassword;
	global $url;
	$requestService='userPayment';
	$client = new soapclientw($url, false,'','', '','');
	$err = $client->getError();
	...
	if( $lys_id == 'webgame')
	{
		$params ='<?xml version="1.0" encoding="GB2312"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><user_account><![CDATA['.$account.']]></user_account><license_id><![CDATA['.$licenseId.']]></license_id><order_id><![CDATA['.$order_id.']]></order_id><fee_money><![CDATA['.$feeMoney.']]></fee_money><subject><![CDATA['.$subject.']]></subject></SOAP-ENV:Body></SOAP-ENV:Envelope>';
	}
	else
	{
		$account = str_replace($_SESSION['prefix'],'',$account);
		$subject = str_replace($_SESSION['prefix'],'',$subject);
		$params ='<?xml version="1.0" encoding="GB2312"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><ly_pid><![CDATA['.$lys_id.']]></ly_pid><user_account><![CDATA['.$account.']]></user_account><license_id><![CDATA['.$licenseId.']]></license_id><order_id><![CDATA['.$order_id.']]></order_id><fee_money><![CDATA['.$feeMoney.']]></fee_money><subject><![CDATA['.$subject.']]></subject><domian><![CDATA['.$_SERVER['HTTP_HOST'].']]></domian><game_code><![CDATA[kdjl]]></game_code></SOAP-ENV:Body></SOAP-ENV:Envelope>';
	}
	$result = $client->call('ManufacturerSearchRequest', $params, $url, 'http://soap.amazon.com');
	return $result;
}

function chekUserlogin($account,$licenseId)
{
	global $udcAcccount;
	global $udcPassword;
	global $url;
	$requestService='checkLicenseId';
	$client = new soapclientw($url, false,'','', '','');
	$err = $client->getError();
	//if ($err) {
	...
	$account=str_replace("]]>","]]&gt;",$account);
	$licenseId=str_replace("]]>","]]&gt;",$licenseId);
	$params ='<?xml version="1.0" encoding="UTF-8"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><user_account><![CDATA['.$account.']]></user_account><license_id><![CDATA['.$licenseId.']]></license_id></SOAP-ENV:Body></SOAP-ENV:Envelope>';
...
function udcLogin($account,$password)
{
	global $udcAcccount;
	global $udcPassword;
	global $url;
	$requestService='userBindCheck';
	$client = new soapclientw($url, false,'','', '','');
	...
	$params ='<?xml version="1.0" encoding="UTF-8"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>'.$udcAcccount.'</Account><Password>'.$udcPassword.'</Password><RequestService>'.$requestService.'</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><user_account><![CDATA['.$account.']]></user_account><user_password><![CDATA['.$password.']]></user_password></SOAP-ENV:Body></SOAP-ENV:Envelope>';
...
class nusoap_base {
    ...
    */
	...
	return
	'<?xml version="1.0" encoding="UTF-8"?><SOAP-ENV:Envelope  xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/"><SOAP-ENV:Header><TransactionID>AFDER2F01125468</TransactionID><Account>passport</Account><Password>BFA861A435E77223385DDE33C333AC76</Password><RequestService>userBindCheck</RequestService></SOAP-ENV:Header><SOAP-ENV:Body><user_account>hykwolf</user_account><user_password>111111</user_password><user_password_type></user_password_type><log_from></log_from><log_ip></log_ip></SOAP-ENV:Body></SOAP-ENV:Envelope>';
    }
...
//curl_setopt($this->ch, CURLOPT_CAINFO, 'f:\php-4.3.2-win32\extensions\curl-ca-bundle.crt');		
	...
	*/
	function send($data, $timeout=0, $response_timeout=30) {
		...
		$this->debug('entered send() with data : '.$data); //wyd
		$this->tryagain = true;
		$tries = 0;
		while ($this->tryagain) {
			$this->tryagain = false;
...
class wsdl extends nusoap_base {
	...
	*/
	function serialize()
	{
		$xml = '<?xml version="1.0" encoding="gb2312"?><definitions';
		foreach($this->namespaces as $k => $v) {
			$xml .= " xmlns:$k=\"$v\"";
		} 
...
class soapclientw extends nusoap_base  {
	...
	* @access   public
	*/
	function call($operation,$params,$namespace='',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		switch(true){
			...
			case ereg('^http',$this->endpoint):
				...
				$this->debug('sending message, length: '.strlen($msg));
					$this->debug('sending message	: '.$msg); //wyd
				...
				if($err = $http->getError()){
					...
					$this->debug('got response, length: '. strlen($this->responseData).' type: '.$http->incoming_headers['content-type']); //wyd
					$this->debug('$this->responseData='.$this->responseData);
