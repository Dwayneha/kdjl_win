...
$rs = $db->getOneRecord("SELECT id,name,nickname,password,secret,from_type,heart_time,secid FROM player WHERE secret = '".md5($_POST['password'])."' AND name= '".$pssport."'");
if (is_array($rs))
{
	if($_REQUEST['from'] == 1)
	{
	...
	if($_REQUEST['from'] == 1)
	{
		...
		$db->getOneRecord($sql);
		$sql = "INSERT INTO chat_login_auth SET uid = '{$_SESSION['id']}',username='{$_SESSION['username']}',nickname='{$user['nickname']}',sid='".session_id()."'";
		$db->getOneRecord($sql);
		echo 'SID'.session_id();
	}
	else
